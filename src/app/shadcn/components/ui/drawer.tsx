...
>(({ className, children, direction = "right", ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ref={ref}
      className={cn(
        "bg-background fixed inset-y-0 z-50 flex h-full w-[400px] flex-col border",
...
function DrawerHeader({
  ...
  return (
    <div
      className={cn(
        "grid gap-1.5 p-4 py-3 text-center sm:text-left",
        className,
      )}
      {...props}
    />
...
function DrawerFooter({
  ...
  return (
    <div
      className={cn("bt-border-base flex flex-col gap-2 px-5 py-3", className)}
      {...props}
    />
  );
}
